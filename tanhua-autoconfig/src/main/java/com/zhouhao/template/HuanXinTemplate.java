...
public class HuanXinTemplate {
    ...
    public User getUser(String huanxinId){
        return null;
    }
    ...
    //发送消息
    public Boolean sendMsg(String senderHxName, String username,String content) {
        try {
            //接收人用户列表
            Set<String> set = CollUtil.newHashSet(username);
            //文本消息
            EMTextMessage message = new EMTextMessage().text(content);
            //发送消息  from：admin是管理员发送
            service.message().send(senderHxName,"users", set,message,null).block();
            return true;
        }catch (Exception e) {
            log.error("发送消息失败~");
        }
